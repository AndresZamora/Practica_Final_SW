...
<html lang="en">
  <head>
	...
	<script>
		...
		function OperacionVer(x){
			var idAlbum= x;
			var xmlhttp = new XMLHttpRequest();
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					location.href="verFotos.php";
				}
			}		
			xmlhttp.open("POST","crearIdAlbum.php",true);
			xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  ...
  <body>
	...
	<div class="container">
		<ol class="breadcrumb">
		  ...
		  <li class="active">Gestion Album</li>
		</ol>
		
		<h3 style="text-align:center"><b>GESTION ALBUM</b></h3><br>		
...
//			$resultado = $enlace->query("SELECT * FROM album WHERE Email = '".$_SESSION['usuarioactual']."'");
			$resultado = $enlace->query("SELECT * FROM album WHERE Email = '".$_SESSION['usuarioactual']."'");
...
<!--					<img src='images/carpeta.png' alt='Album'>-->
					<div class='caption'>
						<p><b>ALBUM ".$fila['ID']."</b></p>
						<p><b>Nombre:</b> ".$fila['Nombre']."</p>
<!--						<p><a href='#' class='btn btn-primary' role='button'>Ver fotos</a>-->
						<button type='button' class='btn btn-primary' role='button' onclick='OperacionVer(".$fila['ID'].")'>Ver fotos</button>
						<button type='button' class='btn btn-default' role='button' onclick='OperacionInsertar(".$fila['ID'].")'>Añadir fotos</button></p>
					</div></div></div>
					");
			}			
